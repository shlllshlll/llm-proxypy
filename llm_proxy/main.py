...
def start_app(args, call_from_main: bool):
    # 日志配置
    # 日志等级
    ...
    if args.debug_server:
        ...
        app = fastapi_server.run_app(args.host, args.port if call_from_main else 0, True)
    else:
        if args.framework == 'flask':
            from llm_proxy import flask_server
            ...
            from llm_proxy import fastapi_server
            app = fastapi_server.run_app(args.host, args.port if call_from_main else 0, False)
...
server_parser.add_argument('--framework', type=str, default='fastapi', choices=['flask','fastapi'],help='server framework')
...
call_from_main = __name__ == "__main__"
...
app = start_app(args, call_from_main)
